...
export const PullRequestSummary = memo(function PullRequestSummary({
  ...
  const prUpdatedLabel = pullRequest.updatedAt
    ...
    : null;
  const prAuthor = pullRequest.author?.login ?? t("gitPanel.unknown");
  ...
  return (
    <section className="diff-viewer-pr" aria-label={t("pullRequestSummary.ariaLabel")}>
      <div className="diff-viewer-pr-header">
          ...
          <div className="diff-viewer-pr-header-actions">
            {hasDiffs && (
              <button
                ...
                aria-label={t("pullRequestSummary.jumpToFirstFile")}
              >
            ...
            {onCheckoutPullRequest ? (
              <button
                ...
                aria-label={t("pullRequestSummary.checkoutBranchAriaLabel", {
              ...
              >
                {isCheckingOut
                  ? t("pullRequestSummary.checkingOut")
                  : t("pullRequestSummary.checkoutBranch")}
              </button>
            ) : null}
          </div>
        </div>
        <div className="diff-viewer-pr-meta">
          ...
          {pullRequest.isDraft && (
            <span className="diff-viewer-pr-pill">{t("gitPanel.draft")}</span>
          )}
        </div>
      </div>
      <div className="diff-viewer-pr-body">
        {prBody ? (
          ...
          <div className="diff-viewer-pr-empty">{t("pullRequestSummary.noDescription")}</div>
        )}
      </div>
      <div className="diff-viewer-pr-timeline">
        <div className="diff-viewer-pr-timeline-header">
          <span className="diff-viewer-pr-timeline-title">{t("pullRequestSummary.activity")}</span>
          <span className="diff-viewer-pr-timeline-count">
            {t("pullRequestSummary.commentCount", { count: sortedComments.length })}
          </span>
        ...
        <div className="diff-viewer-pr-timeline-list">
          {pullRequestCommentsLoading && (
            <div className="diff-viewer-pr-timeline-state">
              {t("pullRequestSummary.loadingComments")}
            </div>
          )}
          {pullRequestCommentsError && (
          ...
          {!pullRequestCommentsLoading &&
            ...
            !sortedComments.length && (
              <div className="diff-viewer-pr-timeline-state">
                {t("pullRequestSummary.noComments")}
              </div>
            )}
          {hiddenCommentCount > 0 && !isTimelineExpanded && (
            <div className="diff-viewer-pr-timeline-divider">
              {t("pullRequestSummary.earlierCommentCount", {
                count: hiddenCommentCount,
              })}
            </div>
          )}
          {visibleComments.map((comment) => {
            const commentAuthor = comment.author?.login ?? t("gitPanel.unknown");
                ...
                <div className="diff-viewer-pr-timeline-content">
                  ...
                  {comment.body.trim() ? (
                    ...
                    <div className="diff-viewer-pr-timeline-text">
                      {t("pullRequestSummary.noCommentBody")}
                    </div>
                  )}
                </div>
              </div>
